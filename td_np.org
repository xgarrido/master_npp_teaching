...
1) PDF: $\Upsigma\vv{F} = m\vv{a} = \frac{d\vv{p}}{dt}$
   ...
   Régime relativiste $m = \gamma{}m \rightarrow \vv{p} = \gamma{}m_{0}\vv{v}$ et donc
   $\text{d}\vv{p}/\text{d}t = \gamma{}m\text{d}\vv{v}/\text{d}t$. Or $\text{d}\vv{v}/\text{d}t
...
2) [@2] Calcul de $t_{0}$
   #+BEGIN_SRC latex
     \begin{align*}
       t_{0} &= \frac{2\pi R}{v} = \frac{2\pi}{qB}\cdot\gamma{}m
     \end{align*}
   #+END_SRC
