...
1) Formule de Gell-Mann et Okubo :
   ...
   \frac{\vv{1}}{2} + \frac{\vv{1}}{2} + \frac{\vv{1}}{2}\]. Les 3 quarks $s$
   sont donc tous dans le même état de spin +1/2 ce qui par définition est
   impossible en raison du principe d'exclusion de Pauli \rightarrow
   introduction de la couleur $RVB$ comme nouveau nombre quantique et justifiant
   l'existence des baryons \Omega^{-}, \Delta^{-} et \Delta^{++}. Chaque quarks
   $s$ existe en 3 couleurs $R,V,B$ d'où $\Omega^{-} = (s_R,s_V,s_B)$. *Symétrie
   de la fonction d'onde globale ??*
...
3) $\Delta^0\rightarrow p + \pi^-$ et $\Lambda^0\rightarrow p + \pi^-\gtrsim$

   #+ATTR_LATEX: :align p{0.4\linewidth}p{0.4\linewidth}
