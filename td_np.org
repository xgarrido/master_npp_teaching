...
#+BEGIN_SRC latex
  \begin{align*}
    I(sortie) &= (m_{p}c^{2} + m_{\pi}c^{2})^{2}\\
    I(entree) &= (E_{p} + E_{\gamma})^{2} - (\vv{p}_{p} + \vv{p}_{\gamma})^{2}c^{2}
  \end{align*}
#+END_SRC

Proton ultra-relativiste E_{p}~T_{p} = p_{p}c
#+BEGIN_SRC latex
  \begin{align*}
    I(entree) &= E_{p}^{2} + E_{\gamma}^{2} + 2E_{p}E_{\gamma} - E_{\gamma}^{2} - E_{p}^{2} - 2E_{\gamma}E_{p}\cos\theta\\
    &= 2E_{p}E_{\gamma}(1 - \cos\theta)
  \end{align*}
#+END_SRC

d'où l'énergie seuil
$$E_{p} \sim T_{p} = \frac{\left(\Upsigma\,m_{finale}c^{2}\right)^{2}}{2E_{\gamma}(1-\cos\theta)}$$

Cas limites: $$\theta\rightarrow0; T_{p}\rightarrow\infty$$ et $$\theta = 180°;
T_{p} = \frac{\left(\Upsigma\,m_{finale}c^{2}\right)^{2}}{4E_{\gamma}}$$

AN: m_{\pi^{0}}c^{2} = 135 MeV et m_{\pi^{+}}c^{2} = 139.6 MeV
#+BEGIN_SRC latex
  \begin{align*}
    T_{p}(\pi^{0}/\pi^{+}) &= \frac{(135/139.6 + 938.3)^{2}\cdot1.6\,10^{-13}}{4\cdot3\cdot1.38\,10^{-23}\cdot2.7} = 4.1\,10^{20}\,MeV\\
  \end{align*}
#+END_SRC
...
diminue de façon significative au cours de sa prop- agation dans l’Univers comme
