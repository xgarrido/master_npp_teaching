...
** Introduction
L'hamiltonien d'un système à $N$ nucléons prend la forme suivante
#+BEGIN_SRC latex
  \begin{equation*}
    \hat{H}=\hat{H}_0 - D\ell^2 - a\vv{\ell}.\vv{s}
  \end{equation*}
#+END_SRC
où $\hat{H}_0$ est l'hamiltonien de l'oscillateur harmonique. $D\ell^2$ traduit
les effets de bord observés expérimentalement et $a\vv{ell}.\vv{s}$ est relatif
au couplage spin-orbite.

On adopte, par ailleurs, les notations héritées de la spectroscopie
atomique/moléculaire (/cf./ Tableau [[tab::quantum_number]]) où $n$ correspond au
...
associé au moment cinétique total soit $\vv{j}=\vv{\ell}+\vv{s}$. Les nucléons
étant des fermions $s=\pm1/2$, le moment cinétique total devient
...
(moment cinétique total) sont possibles. On en déduit les règles d'appariement à
savoir que deux nucléons identiques sur une même couche vont s'apparier de telle
sorte que le moment cinétique soit $I^\pi=0^+$ [fn:7]. On distingue également
les règles suivantes pour les différents types de noyau :
...
- noyau pair-impair :: ces noyaux ont un nucléon célibataire sur une couche
     ...
     \pi=(-1)^\ell$
- noyau impair-impair :: les nucléons célibrataires, 1 proton et 1 neutron, se
...
4)
   ...
   1) \nbsp^{90}_{40}Zr =\nbsp^{88}_{38}Sr + 2$p$ : noyau pair-pair fermé en
      ...
      - 2 protons en $1g\tfrac{9}{2}$ \rightarrow couplage
        ...
        paires sont conservées (noyau pair-pair) d'où $I^\pi=0^+,2^+,4^+,6^+,8^+$
        avec un ordre parmi les niveaux en énergie respecté.
...
[fn:7] la parité est égale à $(-1)^{\ell_1+\ell_2}$ où $\ell_1=\ell_2=\ell$ et
donc $\pi=(-1)^{2\ell}=+1$
